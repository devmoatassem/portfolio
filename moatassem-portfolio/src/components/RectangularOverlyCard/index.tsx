...
export const RectangularOverlyCard: React.FC<{
  ...
  return (
    <motion.article
      className={cn(
        'group/product overflow-hidden w-full h-fit relative shrink-0 p-4 rounded-sm border',
        className,
      )}
    ...
    >
      ...
      <div className="absolute inset-0 h-full w-full opacity-90 group-hover/product:opacity-75 bg-black pointer-events-none transition-opacity duration-300 flex flex-col items-center justify-center px-6" />
      <div className="flex items-center justify-center w-full h-full z-50 md:mt-5 md:group-hover/product:mt-3 transition-all duration-200 ease-in-out">
        <div className="grid lg:grid-cols-2 items-center lg:items-start gap-8 h-fit lg:w-[90%] mx-auto relative z-50 ">
          <div className="flex items-start gap-8">
            ...
            <div>
              <h2 className="text-white text-2xl overflow-clip md:text-4xl font-bold mb-4">{title}</h2>
              {hasTechnologies && (
                    ...
                    {displayTechnologies?.map((category, index) => {
                      if (typeof category === 'object') {
                        ...
                        return (
                          <div
                            key={index}
                            className="inline-block border px-2 py-1  bg-background rounded"
                          >
                            {categoryTitle}
                          </div>
                        )
          ...
          <div>
            ...
            <Link
              href={href}
              className="group-hover/product:shadow-2xl flex items-center gap-2 border border-white/20 bg-white/5 backdrop-blur-sm hover:bg-white/20 px-4 py-2 rounded text-white w-fit z-50 opacity-100 translate-y-0 lg:opacity-0 lg:group-hover/product:opacity-100 lg:translate-y-4 lg:group-hover/product:translate-y-0 transition-all duration-300 lg:delay-75"
            >
